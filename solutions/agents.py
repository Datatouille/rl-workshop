...
class MCAgent:
    ...
    def get_v(self,start_state,epsilon = 0.):
        episode = a.run_episode(start_state,epsilon)
        v = np.sum([episode[i][2] * a.gamma**i for i in range(len(episode))])
        return(v)
    def get_q(self, start_state, first_action, epsilon=0.):
        episode = a.run_episode(start_state,epsilon,first_action)
        q = np.sum([episode[i][2] * a.gamma**i for i in range(len(episode))])
        return(q)
        ...
        for t in range(n_episode):
            ...
            for i in range(len(transitions)):
                if first_visit and ((states[i],actions[i]) not in traversed):
                    ...
                    if lr > 0:
                        self.q[states[i]][actions[i]]+= lr*(g - self.q[states[i]][actions[i]])
                    else:
                        lr = (1/self.n_q[states[i]][actions[i]])
                        self.q[states[i]][actions[i]]+= lr*(g - self.q[states[i]][actions[i]])
                    self.update_policy_q()